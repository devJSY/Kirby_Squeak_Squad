...
namespace sy
{
	...
	void GambleGalaxyScene::Initialize()
	{
		...
		LevelMarkUIAni->CreateAnimation(Tex, L"LevelMarkUI", Vector2(222.f, 995.f), Vector2(96.f, 15.f), Vector2(113.f, 0.f), 1, 1);
		...
		LevelNameUIAni->CreateAnimation(Tex, L"LevelNameUI", Vector2(2.f, 1080.f), Vector2(202.f, 32.f), Vector2(202.f, 0.f), 1, 1);
	...
	void GambleGalaxyScene::Update()
	{
		...
		{
			SceneManager::LoadScene(L"Level8_BossScene");
		}
	...
	void GambleGalaxyScene::SetActiveUI(eStageState type)
	{
	...
	void GambleGalaxyScene::SetClearActiveUI(eStageState type)
	{	
		// 현재 스테이지 클리어 처리
		if (type == eStageState::Boss)
		{
			if (mbActiveUI[0])
			{
				...
				mNumberUI[0]->GetComponent<Animator>()->PlayAnimation(L"Number_DarkNebula");
			}
		}
		...
		scene->SetClearActiveUI(eLevelState::Level8);
	}

	void GambleGalaxyScene::CreateDot()
	{
		/////////// BossStage ///////////
		DotUI* dot;
	...
	void GambleGalaxyScene::CreateStageUI()
	{
		// StageExit UI 생성 
	...
	void GambleGalaxyScene::StageExit()
	{
